...
function duplicateCount(text){
    ...
    const repeatedCharacters = []
      ...
      for(let i = 0; i < text.length; i++) {
          if(repeatedCharacters.includes(text[i])) {continue}
          const numOfTimesRepeated = text.filter(e => text[i] == e).length
          
          if(numOfTimesRepeated > 1) {
              repeatedCharacters.push(text[i])
          }
      }
