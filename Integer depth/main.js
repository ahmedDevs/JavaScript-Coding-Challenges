...
function computeDepth (n){
    const appearedDigits = []
    let multiply = 0
    let depth = 0
    for(let i = 1; appearedDigits.length < 10; i++) {
        multiply = n * i
        multiply = multiply.toString().split('').map(e => !appearedDigits.includes(Number(e)) ? appearedDigits.push(Number(e)) : null)
        depth += 1
    }
    return depth
}