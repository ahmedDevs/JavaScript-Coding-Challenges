...
Continue to do this until a number known to be divisible by 7 is obtained; you can stop when 
...
Task:
Your task is to return to the function seven(m) (m integer >= 0) an array (or a pair, depending on the 
...
//prep
// p  a number integer   number >= 0
// return first number divisible by 7 and the number of steps taken
// examples   seven(371) ---- 37 - (2*1) = 35 % 7 === 0  !! success answer [35,1]
//pesoudo
...
function seven (m) {
    let arr = [];
    let count = 1;
   m = String(m).slice(0, -1) - (String(m).charAt(String(m).length -1)) * 2;
   num2 = m;
    for(let i = 1; m % 7 != 0; i++) {
        num2 = String(m).slice(0, -1) - (String(m).charAt(String(m).length -1)) * 2;
        count += 1;
    }
    arr.push(num2, count);
    return arr;
