/*
There is a secret message in the first six sentences of this kata description. Have you ever felt like 
there was something more being said? Was it hard to figure out that unspoken meaning? Never again! Never 
will a secret go undiscovered. Find all duplicates from our message!
...
If the random tests repeat a word multiple times, it should show up in the secret message only once, based 
on the position of the first time it was duplicated.
...
function findSecretMessage(paragraph) {
    paragraph = paragraph.toLowerCase().split('').filter(e => e !== '.').join('').split(' ')
    const original = []
    const duplicates = []
